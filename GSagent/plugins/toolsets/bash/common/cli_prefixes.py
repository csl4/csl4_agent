...
本模块负责从 ./.GSagent/bash_approved_prefixes.yaml 加载和保存 CLI 已批准的
bash 命令前缀。
...
from GSagent.common import DEFAULT_PREFIXES_FILE
...
# 注意：PREFIXES_FILE 不 import GSagent.config（避免 config → toolsets 循环导入），
#       而从最底层 common 导入统一根常量（./.GSagent/bash_approved_prefixes.yaml）。

PREFIXES_FILE = DEFAULT_PREFIXES_FILE
...
def load_cli_bash_tools_approved_prefixes() -> List[str]:
    """
    从 ./.GSagent/bash_approved_prefixes.yaml 加载已批准前缀。
...
def save_cli_bash_tools_approved_prefixes(prefixes: List[str]) -> None:
    """
    将已批准前缀保存到 ./.GSagent/bash_approved_prefixes.yaml。
