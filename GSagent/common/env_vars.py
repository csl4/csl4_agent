...
目录、服务端设置）都由 `GSagent.config.Config` 负责——请勿在此重复声明
这些常量；重复的默认值会漂移失联。
...
#   GSagent.config.Config】——不要在这里重复声明，否则两份默认值会漂移失联。
# 这里只放「不能等 Config」的 import 期常量：LOG_LEVEL 在 GSagent.utils.log
#   import 时就已被读取（那时 Config 还没构造）。
# =========================================================

# --- Logging ---
# LOG_LEVEL 在 GSagent.utils.log import 期就被读取（那时 Config 还没构造），保持冻结常量。
...
def log_dir() -> str:
    """日志目录：AGENT_LOG_DIR 覆盖；空串表示用默认 ./.GSagent/logs。"""
