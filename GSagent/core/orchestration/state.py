...
彻底 langgraph 化（用户要求）：删掉手写的暂停状态字段与事件缓冲通道，
审批/前端暂停改由 langgraph ``interrupt()`` 承载（人在回环），恢复用
``Command(resume=...)`` + checkpointer（thread_id 固定，真正断点续跑）；
渲染事件改由节点经 ``get_stream_writer()`` 推 langgraph custom 流。

保留字段：
- ``messages``：langchain ``BaseMessage`` 列表，``add_messages`` 追加式合并。
- ``terminated``：熔断/取消/输入拦截（max_steps/cancelled/blocked）——这些是
  图提前结束信号，非暂停，不走 interrupt。
- ``last_tool_calls`` / ``prev_tool_calls`` / ``no_progress_streak``：死循环检测
  （宪法 10.1）。
- 回合配置（``request_context``/``cancel_event``）：每次回合经初始状态传入，
  使图编译一次、每回合复用。
"""
...
class GraphState(TypedDict, total=False):
    ...
    tool_number: int  # 工具编号偏移（跨 interrupt/resume 保持连续）
